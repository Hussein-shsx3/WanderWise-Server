...
exports.generateTravelItinerary = void 0;
 ...
 */
const parseAIResponse = (response, numberOfDays, destination) => {
    const lines = response.split("\n");
    const dayActivities = Array(numberOfDays)
        .fill(null)
        .map(() => []);
    let currentDay = 0;
    let summary = "";
    ...
    for (const line of lines) {
        ...
        if (currentDay >= 0 && currentDay < numberOfDays && trimmedLine.length > 0) {
            ...
            if (trimmedLine.toLowerCase().includes("day") ||
...
const estimateCost = (text) => {
    const lower = text.toLowerCase();
    if (lower.includes("luxury") || lower.includes("fine dining"))
        return 150;
    if (lower.includes("restaurant") || lower.includes("dinner"))
        return 60;
    if (lower.includes("cafe") || lower.includes("lunch"))
        return 30;
    if (lower.includes("museum") || lower.includes("tour"))
        return 25;
    if (lower.includes("hike") || lower.includes("walk"))
        return 0;
    if (lower.includes("activity"))
        return 50;
    return 0;
};
const generateTravelItinerary = async (destination, startDate, endDate, travelStyle, budget, preferences, numberOfDays) => {
    try {
        if (!OPENROUTER_API_KEY) {
            throw new errorMiddleware_1.AppError("OpenRouter API key not configured", 500, "CONFIG_ERROR");
        }
...
Format your response in a structured way that can be parsed. Use clear day-by-day breakdown.`;
        ...
        if (!response.data.choices || !response.data.choices[0]?.message?.content) {
            throw new errorMiddleware_1.AppError("Invalid response format from AI service", 500, "AI_SERVICE_ERROR");
        }
        ...
        if (axios_1.default.isAxiosError(error)) {
            ...
            if (status === 405) {
                throw new errorMiddleware_1.AppError(`OpenRouter API Error: ${status} ${statusText}. Please check the API endpoint and credentials.`, 500, "AI_SERVICE_ERROR");
            }
            if (status === 401 || status === 403) {
                throw new errorMiddleware_1.AppError("OpenRouter API authentication failed. Invalid API key.", 500, "AI_SERVICE_ERROR");
            }
            if (status === 429) {
                throw new errorMiddleware_1.AppError("OpenRouter API rate limit exceeded. Please try again later.", 429, "RATE_LIMIT_ERROR");
            }
            throw new errorMiddleware_1.AppError(`OpenRouter API Error: ${error.message}`, 500, "AI_SERVICE_ERROR");
        }
        throw new errorMiddleware_1.AppError("Failed to generate itinerary with AI", 500, "AI_SERVICE_ERROR");
    }
};
exports.generateTravelItinerary = generateTravelItinerary;
